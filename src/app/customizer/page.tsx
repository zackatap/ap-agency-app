...
export default function CustomizerPage() {
  ...
  const locationId = useMemo(() => {
    return (
      ...
      searchParams.get("locationId") ??
      process.env.NEXT_PUBLIC_CUSTOMIZER_LOCATION_ID ??
      ""
    ).trim();
  }, [searchParams]);
          ...
          <main className="rounded-2xl border border-white/10 bg-slate-900/60 p-4">
            ...
            <div className="rounded-xl border border-white/10 bg-slate-950/40 p-2">
              <iframe
                ...
                style={{ width: "100%", minHeight: `${activeCampaign.height}px`, border: "none", borderRadius: "10px" }}
          ...
          <aside className="space-y-4">
            <section className="rounded-2xl border border-white/10 bg-slate-900/60 p-4">
              ...
              <p className="mt-1 text-xs text-slate-400">
                Base page test: workflows containing <span className="font-semibold text-sky-300">pain</span>, sorted by name.
              </p>
              {!locationId && (
                <p className="mt-3 rounded-lg border border-amber-300/20 bg-amber-400/10 px-3 py-2 text-xs text-amber-200">
                  No location ID detected. Add <code>?locationid=...</code> to fetch workflows.
                </p>
              )}
              ...
              {locationId && active === "base" && !workflowsLoading && !workflowsError && workflows.length === 0 && (
                <p className="mt-3 text-sm text-slate-300">No matching workflows found.</p>
              )}
